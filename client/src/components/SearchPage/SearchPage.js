								...
								{({ loading: loadingReviews, data: reviewData, error: reviewError }) => {
									...
									projectArray = projectArray.map((project) => {
										if (project.rating.length > 1) {
											return (project = {
												...project,
												rating: parseFloat(math.mean(project.rating.slice(1)).toFixed(2))
											});
										} else {
											return (project = {
												...project,
				...
				<div className="card-container">
					{this.props.projects
						.map(({ id, name, titleImg, rating, User, category }) => {
							...
							if (rating.length > 1) meanRating = parseFloat(math.mean(rating.slice(1)).toFixed(2));
							if (rating.length === 1) meanRating = parseFloat(math.mean(rating).toFixed(2));

							const stars = [];

							for (let i = 0; i < Math.round(meanRating); i++) {
								stars.push(<img src={star} className="stars" alt="star" key={i} />);
							}

							return (
								<div className="card" key={id}>
									<div>
										<img className="projectImage" src={`${titleImg}`} alt="project" />
									...
									<Query query={getUsers} key={review.id}>
										{({ loading, data, error, refetch }) => {
											...
											if (data) {
												...
												let rev = user[0].ReviewList.filter((r) => r.id === review.id)[0];
