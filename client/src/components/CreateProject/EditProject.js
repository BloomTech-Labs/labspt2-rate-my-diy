...
class EditProject extends Component {
	constructor(props) {
		...
		const userPull = this.props.users.filter((user) => user.email === json.email);
		...
		const { name, category, timestamp, titleImg, titleBlurb, steps, id } = this.props.project;
		let newSteps = [];
		if (typeof steps === 'string') newSteps = JSON.parse(steps).concat([ { type: '', body: '' } ]);
		if (!typeof steps === 'string') newSteps = steps.concat([ { type: '', body: '' } ]);
	...
	addImage = (img) => {
		const steps = this.state.project.steps.filter((step) => step !== { type: '', body: '' });
	...
	finalize = async (e) => {
		...
		try {
			...
			const filter = await steps.filter((step) => step.type !== '' && step.body !== '');

			const string = await JSON.stringify(filter);

			const date = await new Date(Date.now());

			const { name, category, titleImg, titleBlurb, id } = await this.state.project;
	...
	render() {
		...
		if (this.state.project.steps != null && typeof this.state.project.steps === 'object') {
			return (
				<div className="projectInfoEdit">
					<form className="projectEditForm">
						...
						<div className="project-Flex">
							<h2>Project Title:</h2>
							<input type="text" name="name" value={this.state.project.name} onChange={this.textChange} />
						</div>
						<div>
									...
									if (step.type === 'img') {
										return (
											<div className="editImageSection" key={idx}>
												...
												<button onClick={this.deletePhoto(idx)}>Delete Photo</button>
											</div>
										);
									} else {
										return (
					...
					{(editProject, { loading, error, data }) => {
                      ...
                      <div className="steps-container">
											...
											{this.state.submitDisabled ? (
												<button className="submitButton" type="button" onClick={this.finalize}>
													Finalize
												</button>
											) : (
												<button
